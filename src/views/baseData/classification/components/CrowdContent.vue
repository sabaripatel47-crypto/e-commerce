<template>
  <div class="CategoryContent-page">
    <el-form class="page-search" inline ref="page-filter">
      ...
      <br />-->
      <el-button type="primary" icon="el-icon-plus" @click="addCrowd({uuid:'0'})">新增</el-button>
    ...
    >
      <el-table-column prop="cateName" width="500" label="人群名称"></el-table-column>
      ...
      <el-table-column label="操作" width="180" align="center">
        <template v-slot="{row}">
          <el-button type="text" @click="addCrowd(row)">新增</el-button>
    ...
    >
      <el-form ref="addDialogRef" :model="addReqParams" :rules="addRules" label-width="100px">
        <el-form-item v-if="addReqParams.parentName" label="上级人群：">
          <span>{{ addReqParams.parentName }}</span>
        </el-form-item>
        <el-form-item label="人群名称：" prop="cateName">
          <el-input
...
export default {
  ...
  methods: {
    ...
    async submitForm() {
      ...
      if (this.isEdit) {
        const res = await updateCrowd(this.addReqParams)
      } else {
        const res = await addCrowd(this.addReqParams)
      }
    ...
    async getDataList() {
      ...
      const res = await getCrowdList(this.reqParams)
