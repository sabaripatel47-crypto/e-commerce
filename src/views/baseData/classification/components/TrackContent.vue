...
<template>
  <div class="TrackContent-page">
    <el-form class="page-search" inline ref="page-filter">
      ...
      <br />-->
      <el-button type="primary" icon="el-icon-plus" @click="addTrack({uuid:'0'})">新增</el-button>
    ...
    >
      <el-table-column prop="cateName" width="500" label="赛道名称"></el-table-column>
      ...
      <el-table-column label="操作" width="180" align="center">
        <template v-slot="{row}">
          <el-button type="text" @click="addTrack(row)">新增</el-button>
          <el-button type="text" @click="editTrack(row)">修改</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 新增弹层 -->
    ...
    >
      <el-form ref="addDialogRef" :model="addReqParams" :rules="addRules" label-width="100px">
        <el-form-item v-if="addReqParams.parentName" label="上级赛道：">
          <span>{{ addReqParams.parentName }}</span>
        </el-form-item>
        <el-form-item label="赛道名称：" prop="cateName">
          <el-input
...
export default {
  ...
  methods: {
    ...
    async submitForm() {
      ...
      if (this.isEdit) {
        const res = await updateTrack(this.addReqParams)
      } else {
        const res = await addTrack(this.addReqParams)
      }
    ...
    async getDataList() {
      ...
      const res = await getTrackList(this.reqParams)
