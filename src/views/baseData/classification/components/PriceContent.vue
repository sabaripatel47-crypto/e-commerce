<template>
  <div class="CategoryContent-page">
    <el-form class="page-search" inline ref="page-filter">
      ...
      <br />-->
      <el-button type="primary" icon="el-icon-plus" @click="addPrice({uuid:'0'})">新增</el-button>
    ...
    >
      <el-table-column prop="cateName" width="500" label="价格带名称"></el-table-column>
      ...
      <el-table-column label="操作" width="180" align="center">
        <template v-slot="{row}">
          <el-button type="text" @click="addPrice(row)">新增</el-button>
    ...
    >
      <el-form ref="addDialogRef" :model="addReqParams" :rules="addRules" label-width="130px">
        <el-form-item v-if="addReqParams.parentName" label="上级价格带：">
          <span>{{ addReqParams.parentName }}</span>
        </el-form-item>
        <el-form-item label="价格带名称：" prop="cateName" >
          <el-input
            class="el-form-item-width40"
            v-model="addReqParams.cateName"
            placeholder="请输入价格带名称"
          ></el-input>
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      addRules: {
        cateName: [
          { required: true, message: '请输入价格带名称', trigger: 'blur' },
        ],
  ...
  methods: {
    ...
    async submitForm() {
      ...
      if (this.isEdit) {
        const res = await updatePrice(this.addReqParams)
      } else {
        const res = await addPrice(this.addReqParams)
      }
    ...
    async getDataList() {
      ...
      const res = await getPriceList(this.reqParams)
