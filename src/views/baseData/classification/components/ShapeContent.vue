<template>
  <div class="CategoryContent-page">
    <el-form class="page-search" inline ref="page-filter">
      ...
      <br />-->
      <el-button type="primary" icon="el-icon-plus" @click="addShape({uuid:'0'})">新增</el-button>
    ...
    >
      <el-table-column prop="cateName" width="500" label="器型名称"></el-table-column>
      ...
      <el-table-column label="操作" width="180" align="center">
        <template v-slot="{row}">
          <el-button type="text" @click="addShape(row)">新增</el-button>
    ...
    >
      <el-form ref="addDialogRef" :model="addReqParams" :rules="addRules" label-width="100px">
        <el-form-item v-if="addReqParams.parentName" label="上级器型：">
          <span>{{ addReqParams.parentName }}</span>
        </el-form-item>
        <el-form-item label="器型名称：" prop="cateName">
          <el-input
...
export default {
  ...
  data() {
    return {
      ...
      addRules: {
        cateName: [
          { required: true, message: '请输入器型名称', trigger: 'blur' },
        ],
  ...
  methods: {
    ...
    async submitForm() {
      ...
      if (this.isEdit) {
        const res = await updateShape(this.addReqParams)
      } else {
        const res = await addShape(this.addReqParams)
      }
    ...
    async getDataList() {
      ...
      const res = await getShapeList(this.reqParams)
