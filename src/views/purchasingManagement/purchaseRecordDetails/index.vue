  ...
  <div class="app-container">
    <div>
      <div class="search-form">
        <el-form inline>
          ...
          <el-form-item label="安全库存警戒线:" style="margin-right: 20px">
            <el-select v-model="reqParams.safeLone" clearable placeholder="请选择">
              <el-option
      ...
      >
        ...
        <el-table-column label="本月运营申请计划" align="center">
          <template slot-scope="scope">
            <el-button
              v-if="scope.row.applyPlanNum != null && scope.row.applyPlanNum != 0"
              type="text"
              size="small"
        ...
        <el-table-column label="上月运营申请计划" align="center">
          <template slot-scope="scope">
            <el-button
              v-if="scope.row.lastApplyPlanNum != null && scope.row.lastApplyPlanNum != 0"
              type="text"
              size="small"
              @click="getPurchaseDetails(scope.row.skuUuid,scope.row.lastMonth)"
        ...
        <el-table-column label="下月运营申请计划" align="center">
          <template slot-scope="scope">
            <el-button
              v-if="scope.row.nextMonthNum != null && scope.row.nextMonthNum != 0"
              type="text"
              size="small"
              @click="getPurchaseDetails(scope.row.skuUuid,scope.row.nextMonth)"
    ...
    <TrendChart :show-dialog.sync="trendDialogVisible" :sku-uuid="skuUuid" />
...
import { getSafetyStockList } from '@/api/cargoPulling/cargoRecordDetail'
...
export default {
  ...
  methods: {
    ...
    async generateRemainingPurchasePlan() {
      const res = await autoPurchaseRest()
      this.addPurchaseOrderLists = res.data
      this.openDateDialog()
    },
    // 追加采购
