<template>
  <div class="page_configure">
    ...
    <el-dialog title="审批" :visible.sync="approvalDialog" width="40%" :before-close="approvalClose">
      <el-form ref="addRef" label-width="100px" :model="auditInsertDTO" :rules="addRules">
        ...
        <el-form-item label="审核：" prop="audit">
          <el-radio-group v-model="auditInsertDTO.audit">
            <el-radio :label="1">通过</el-radio>
            <el-radio :label="2">不通过</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item v-if="rowData.feedback === '是'" label="反馈：">
          <el-input
            type="textarea"
        ...
        <el-form-item v-if="rowData.isAllocation === 1" label="分配：">
          <el-form :inline="true">
            <el-form-item style="width: 10vw;">
              <el-cascader
                v-model="auditInsertDTO.allocationId"
                :options="options"
            ...
            <el-form-item>
              <el-form :inline="true">
                <span>时效性：</span>
                <el-form-item style="width: 5vw;">
                  <el-select v-model="auditInsertDTO.day" placeholder>
                    <el-option v-for="one in 6" :key="one" :label="one" :value="one"></el-option>
                  </el-select>
                </el-form-item>
                <el-form-item style="width: 5vw;">
                  <el-select v-model="auditInsertDTO.timeQty" placeholder>
...
<script>
import { getList, submitApp } from '@/api/routine/approve.js'
import { department, flowDetail } from '@/api/routine/configure.js'
...
export default {
  ...
  data() {
    return {
      ...
      flowUserName: '',
      addRules: {
        audit: [{ required: true, message: '请选择', trigger: 'change' }],
      },
      // 分页入参
      pageReqPrams: {
        pageNum: 1,
  ...
  methods: {
    ...
    async submitApprove() {
      ...
      /* 判断是不是最后一个审核人 */
      // 判断是不是有分配权限
      if (this.rowData.isAllocation === 0) {
        if (
          this.rowData.sort === appList.data.deptList?.length &&
          this.auditInsertDTO.audit === 1
        ) {
      ...
      this.auditInsertDTO.timeliness =
        this.auditInsertDTO.day + this.auditInsertDTO.timeQty
      delete this.auditInsertDTO.day
      delete this.auditInsertDTO.timeQty
      if (Array.isArray(this.auditInsertDTO.allocationId)) {
        this.auditInsertDTO.allocationId =
          +this.auditInsertDTO.allocationId[
            this.auditInsertDTO.allocationId?.length - 1
          ]
      }
      if (this.fileList.length > 0) {
        ...
        for (let i = 0; i < this.fileList.length; i++) {
          ...
          if (i === 0) {
            this.auditInsertDTO.feedback =
              this.auditInsertDTO.feedback + res.data
          } else {
            this.auditInsertDTO.feedback =
              this.auditInsertDTO.feedback + ',' + res.data
          }
    ...
    async getFlowList() {
      this.loading = true
      const res = await getList({ audit: this.audit, ...this.pageReqPrams })
    ...
    async goApprove(row) {
      ...
      res.data.list.map((item) => {
        if (item && item.allocationId !== null) {
          num = num + 1
        }
      })
      if (row.sort + num !== res.data.list.length + 1) {
        this.$message.warning('暂时无法审批')
      } else {
