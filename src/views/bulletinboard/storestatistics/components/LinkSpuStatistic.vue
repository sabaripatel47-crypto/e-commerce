  ...
  <div class="statistics">
    <div class="statistics-container">
      <el-table v-loading="tableLoading" :data="tableDataList" border style="width: 100%">
        ...
        >
          <template slot-scope="{row}">
            <div>{{ formatTableItemCode(row.levelName) }}</div>
          </template>
        </el-table-column>
        <el-table-column
          prop="time"
        ...
        <el-table-column label="操作" width="80" show-overflow-tooltip>
          <template slot-scope="scope">
            <el-button
              ...
              @click="getLinkSpuLineOptions(tableDataList , scope.row)"
            >趋势</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog
      width="70%"
      title="链接趋势"
      :visible.sync="showLinkSpuLine"
      append-to-body
      style="overflow-x: auto;"
    >
      <line-chart
        v-if="showLinkSpuLine"
...
export default {
  ...
  methods: {
    // 链接趋势图
    getLinkSpuLineOptions(data, row) {
      this.showLinkSpuLine = true
      ...
      // data.map((item, index) => {
      //   // option.legend.data.push(row.storeName,)
      //   option.series.push({
      //     name: row.spaceName,
      //     type: 'line',
      //     data: [...item.origin],
