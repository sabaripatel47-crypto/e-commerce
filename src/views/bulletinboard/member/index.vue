      ...
      <el-col :span="17">
        ...
        <div class="head-container">
          <el-form class="page-search" inline ref="page-filter">
            ...
            <el-form-item label="业务员：" style="margin-right: 20px;">
              <el-cascader
                filterable
                v-model="deptReports.userUuid"
                :options="options"
        ...
        <div class="right-table">
          <div>
            ...
            <el-table v-loading="numTableLoading" :data="tableDataOrder" border style="width: 100%">
              <el-table-column label="序号" class="center-align">
                <template slot-scope="scope">{{ scope.$index + 1 }}</template>
              </el-table-column>
              <el-table-column label="业务员" class="center-align">
                <template slot-scope="scope">{{ `${scope.row.manager}` }}</template>
              </el-table-column>
              <el-table-column label="商品id" class="center-align">
                <template slot-scope="scope">{{ scope.row.linkId }}</template>
              </el-table-column>
              <el-table-column label="数量" class="center-align">
                <template slot-scope="scope">{{ scope.row.quantity }}</template>
              </el-table-column>
              <el-table-column label="金额" class="center-align">
                <template slot-scope="scope">{{ scope.row.paymentPrice }}</template>
              </el-table-column>
            </el-table>
          </div>
...
<script>
import { getDepartmentUserList } from '@/api/publicDepartment'
...
export default {
  ...
  data() {
    return {
      selectedPlatforms: [], // 用于存储选中的平台
      selectedStores: [], // 用于存储选中的店铺
      //平台下拉框
      platformList: [],
      //店铺下拉框
      storeList: [],
  ...
  methods: {
    ...
    //搜索框
    updateDeptReports() {
      if (Array.isArray(this.deptReports.userUuid)) {
        this.deptReports.userUuid =
          this.deptReports.userUuid[this.deptReports.userUuid.length - 1]
      }
