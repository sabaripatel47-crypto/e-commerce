  ...
  <div class="audit-subtable-page">
    ...
    <el-table v-loading="tableLoading" border max-height="400" :data="tableData">
      ...
      <el-table-column label="本地仓" width="75" prop="stockNum" align="center">
        <template v-slot="{ row }">{{ row.stockNum || 0 }}</template>
      </el-table-column>
...
<script>
import { fetch_sonDetailData } from '@/api/support/audit/index'
import {
  categoryList,
...
export default {
  ...
  data() {
    return {
      ...
      productDetailSkuUuid: '', // SKU UUID，用于点击帮扶复核时传递
    }
  },
  watch: {
    helpSupportUuid: {
  ...
  methods: {
    ...
    async getList() {
      ...
      try {
        ...
        const res = await fetch_sonDetailData(params)
